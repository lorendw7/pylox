...
And by **Tier F** it gains a *functional* flavor — functions as values, lambdas,
pipelines, and pattern matching:

```lox
# Functions are ordinary values (Tier F)
let double = fn(x) { x * 2 }

# Higher-order functions chained into a pipeline with |>
let result = [1, 2, 3, 4]
    |> map(fn(n) { n * n })        # square each   -> [1, 4, 9, 16]
    |> filter(fn(n) { n > 4 })     # keep > 4      -> [9, 16]
    |> reduce(0, fn(a, b) { a + b })
print(result)                       # 25

# Pattern matching over a sum type
let area = fn(shape) {
    match shape {
...
By the end this same language supports functions, arrays, dictionaries, closures, friendly
error messages, **and** the functional features above — all of which **you** will have
implemented.
...
| **First-class function** | 一等函数 | A function that can be stored in variables, passed as an argument, and returned. 能像普通值一样传递的函数。 |
| **Higher-order function** | 高阶函数 | A function that takes or returns another function (e.g. `map`). 接收或返回函数的函数。 |
| **Pure function** | 纯函数 | Same input always gives same output, with no side effects. 同输入必同输出、无副作用。 |
| **Immutability** | 不可变性 | Values that cannot be changed after creation. 创建后不能被修改的值。 |
| **Pattern matching** | 模式匹配 | Branching by *destructuring* a value's shape, not just testing booleans. 按值的结构来分支并解构。 |
| **Algebraic data type (ADT)** | 代数数据类型 | A type built from tagged variants, e.g. `Circle(r) | Rect(w, h)`. 由“带标签的多种形态”组成的类型。 |
| **Currying / partial application** | 柯里化 / 部分应用 | Turning a multi-arg function into a chain of one-arg functions; fixing some args early. 把多参函数拆成一串单参函数。 |
| **Tail call / TCO** | 尾调用 / 尾调用优化 | A call in tail position reused as the current frame, so deep recursion won't overflow. 尾位置的调用复用当前栈帧，使深递归不爆栈。 |

---

## 4. Learning Roadmap / 学习路线

The project is organized into **six tiers**, each a meaningful milestone. There are two
tracks once the language runs:

- **Performance track** — Tier D is the leap from a tree-walking interpreter into a real
  **bytecode compiler + virtual machine** (the architecture of CPython and the JVM); Tier E
  adds a **JIT**, compiling hot code at runtime the way V8, PyPy, and HotSpot do.
- **Expressiveness track** — Tier F turns pylox into a **functional language**: first-class
  functions, lambdas, pattern matching, pipelines, and tail-call optimization.

> 🇨🇳 **两条支线的关系：** D/E 是“跑得更快”（实现技术），F 是“写得更爽”（语言表达力）。
> Tier F 全部跑在你 Tier A–C 的树遍历解释器 + Tier D 闭包（Lesson 12）之上，**不依赖虚拟
> 机**——所以学完闭包后，你可以随时插入 Tier F，先玩函数式，再回头做 VM/JIT，顺序很自由。
> 这本身就是个重要认知：**“语言有什么特性”和“语言跑得多快”是两件独立的事**。
...
### Tier F — Functional Programming: a language you'll love to write / 函数式编程

*Builds on Lesson 12 (closures) and runs on the tree-walking interpreter — do it any time
after Tier D's closures.* 建立在第 12 课（闭包）之上，跑在树遍历解释器上，学完闭包即可开做。

| Lesson | Topic | What you'll learn | Status |
| --- | --- | --- | --- |
| 21 | Higher-order functions | First-class functions in earnest; building `map` / `filter` / `reduce` | ⏳ |
| 22 | Lambdas | Anonymous-function syntax (`fn(x) { ... }`) and closures capturing their environment | ⏳ |
| 23 | Expression-oriented | Make `if` and blocks *return values* (`let y = if c { a } else { b }`) | ⏳ |
| 24 | Immutability & purity | Immutable bindings, pure functions, and why side-effect-free code is easier to reason about | ⏳ |
| 25 | Pattern matching & ADTs | A `match` expression; sum types like `Circle(r) | Rect(w, h)` with destructuring | ⏳ |
| 26 | Pipelines & composition | The `|>` pipe operator, function composition, currying & partial application | ⏳ |
| 27 | Tail-call optimization | Replace loops with recursion *without* overflowing the stack — the FP way to iterate | ⏳ |
...
>
> 🇨🇳 **Tier F 是“函数式编程入门”最好的方式**：你不是在*学*别人语言的函数式特性，而是
> *亲手把它们造进自己的语言里*——`map`、lambda、模式匹配、柯里化、尾递归优化，每一个你都
> 会从“它在解释器里到底怎么实现”这个角度彻底搞懂，这是看十本书都换不来的理解。
...
**Tier F — Functional Programming**
- [ ] Lesson 21: Higher-order functions (map / filter / reduce)
- [ ] Lesson 22: Lambdas (anonymous functions)
- [ ] Lesson 23: Expression-oriented (`if`/blocks return values)
- [ ] Lesson 24: Immutability & pure functions
- [ ] Lesson 25: Pattern matching & algebraic data types
- [ ] Lesson 26: Pipelines, composition & currying
- [ ] Lesson 27: Tail-call optimization
...
- **Functional programming** — for Tier F, *Structure and Interpretation of Computer
  Programs* (SICP, free online) builds a language and embraces FP; trying a real FP language
  like **Elm, OCaml, or Haskell** alongside makes the ideas click. 配合 Tier F：SICP（免费）
  与一门真正的函数式语言（Elm/OCaml/Haskell）一起看，概念会瞬间通透。
